...
export default function App() {
  ...
  const [settings, setSettings] = useState<AppSettings>({ theme: "system", recentPaths: [], fullDiskAccessSetupComplete: false });
  ...
  useEffect(() => {
    ...
    }).catch((reason) => setError(String(reason))).finally(() => setSettingsLoaded(true));
    api.getPlatform().then(setPlatform).catch((reason) => setError(String(reason)));
  ...
  async function completeFullDiskAccessSetup() {
    const next = { ...settings, fullDiskAccessSetupComplete: true };
    try {
      await api.saveSettings(next);
      setSettings(next);
      setNotice("Full Disk Access setup acknowledged. FilePilot will now stay out of the way unless you reopen it from Settings.");
    } catch (reason) {
      setError(String(reason));
    }
  }
  ...
  function renderModule() {
    ...
    switch (activeModule) {
      ...
      case "settings": return <SettingsPage settings={settings} setSettings={setSettings} setNotice={setNotice} setError={setError} platform={platform} openFullDiskAccessSettings={openFullDiskAccessSettings} />;
  ...
  return (
    <div className="app-shell">
      ...
      {settingsLoaded && platform === "macos" && !settings.fullDiskAccessSetupComplete && <FullDiskAccessOnboarding onOpenSettings={openFullDiskAccessSettings} onComplete={completeFullDiskAccessSetup} />}
    </div>
  );
}
...
export function FullDiskAccessOnboarding({ onOpenSettings, onComplete }: { onOpenSettings: () => Promise<void>; onComplete: () => Promise<void> }) {
  return <div className="onboarding-backdrop"><section className="onboarding-dialog" role="dialog" aria-modal="true" aria-labelledby="full-disk-access-title"><span className="card-kicker">ONE-TIME MACOS SETUP</span><h2 id="full-disk-access-title">See more of your Mac’s storage</h2><p>Full Disk Access lets FilePilot read protected local folders while it scans. It does not grant administrator access, network access, or permission to delete protected files.</p><ol><li>Open <strong>Full Disk Access</strong> in System Settings.</li><li>Turn on <strong>FilePilot</strong>. Use the <strong>+</strong> button if it is not listed.</li><li>Quit and reopen FilePilot, then run the System Data scan again.</li></ol><p className="form-note">macOS does not let FilePilot verify this setting. Some removable drives and app-specific folders can still require separate approval.</p><div className="onboarding-actions"><button className="button primary" onClick={() => void onOpenSettings()}>Open Full Disk Access</button><button className="button secondary" onClick={() => void onComplete()}>I’ve finished setup</button><button className="button ghost" onClick={() => void onComplete()}>Continue without it</button></div></section></div>;
}
...
function SystemDataPage(props: CommonProps) {
  ...
  const reviewBytes = output?.items
    ...
    .reduce((total, item) => total + item.sizeBytes, 0) ?? 0;
  return <><PageIntro title="System Data" description="Trace named storage contributors, understand their owner, and separate reviewable files from data macOS should manage itself."><div className="page-actions"><button className="button primary" onClick={() => analyze(output?.scopePath)}>{output?.scopePath ? "Refresh this location" : "Analyze System Data"}</button>{output?.scopePath && <button className="button secondary" onClick={() => analyze()}>Analyze all locations</button>}</div></PageIntro><section className="system-data-notice"><strong>Clear scope</strong><span>FilePilot reports non-overlapping on-disk locations; it does not try to reproduce macOS Storage Settings’ System Data number. It only offers recoverable Trash cleanup for selected user cache and log folders.</span></section>{output ? <><ReportHeader count={`${output.scopePath ? `Detail · ${output.scopePath} · ` : ""}${output.items.length} largest contributors · ${formatBytes(output.totalBytes)} found`} output={outputEntry} exportOutput={props.exportOutput} /><section className="stats-grid"><Stat label="Targeted storage found" value={formatBytes(output.totalBytes)} detail="named local locations only" /><Stat label="Likely reviewable" value={formatBytes(reviewableBytes)} detail="caches, logs, and temporary data" /><Stat label="Needs owner review" value={formatBytes(reviewBytes)} detail="app state, backups, and developer data" /><Stat label="Volume free" value={output.volume?.freeBytes ? formatBytes(output.volume.freeBytes) : "Unavailable"} detail={output.volume?.apfsSnapshotCount !== undefined ? `${output.volume.apfsSnapshotCount} APFS snapshots reported` : "APFS container space"} /></section><div className="storage-list">{output.items.map((item) => <StorageItemCard key={`${item.path}-${item.label}`} item={item} onAnalyze={analyze} onCleanup={cleanup} />)}</div>{output.notes.length > 0 && <div className="info-list"><strong>How to read this report</strong>{output.notes.map((note) => <span key={note}>• {note}</span>)}</div>}{output.warnings.length > 0 && <WarningList warnings={output.warnings} />}</> : <EmptyState title="No System Data report yet" description="Run the analyzer to inspect named user-library, shared-library, and system-working-data sources without changing them." />}</>;
}
...
export function StorageItemCard({ item, nested = false, onAnalyze, onCleanup }: { item: StorageItem; nested?: boolean; onAnalyze: (path: string) => void; onCleanup: (item: StorageItem) => Promise<void> }) {
  return <article className={`storage-item ${nested ? "nested" : ""}`}><div className="storage-item-heading"><div><strong>{item.label}</strong><span className="storage-path">{item.path}</span></div><div className="storage-actions">{item.isDirectory && <button className="button ghost" onClick={() => onAnalyze(item.path)}>Inspect</button>}{item.cleanupAllowed && <button className="button ghost danger" onClick={() => void onCleanup(item)}>Move to Trash</button>}<div className="storage-size">{item.sizeKnown ? formatBytes(item.sizeBytes) : "Size not reported"}</div></div></div><div className="storage-meta"><span className="category-pill">{storageCategoryLabel(item.category)}</span><span className={`assessment-pill ${item.assessment}`}>{assessmentLabel(item.assessment)}</span></div><p className="storage-reason"><strong>Why it takes space:</strong> {item.reason}</p><p className="storage-recommendation"><strong>Suggested next step:</strong> {item.recommendation}</p>{item.children.length > 0 && <details className="storage-children"><summary>Show largest child locations</summary>{item.children.map((child) => <StorageItemCard key={child.path} item={child} nested onAnalyze={onAnalyze} onCleanup={onCleanup} />)}</details>}</article>;
}
...
function SettingsPage({ settings, setSettings, setNotice, setError, platform, openFullDiskAccessSettings }: { settings: AppSettings; setSettings: (settings: AppSettings) => void; setNotice: (notice: string) => void; setError: (error: string) => void; platform: string; openFullDiskAccessSettings: () => Promise<void> }) { return <><PageIntro title="Settings" description="Keep FilePilot comfortable and predictable across launches." /><section className="form-card settings-card"><label>Appearance<select value={settings.theme} onChange={(event) => setSettings({ ...settings, theme: event.target.value as Theme })}><option value="system">Follow system</option><option value="light">Light</option><option value="dark">Dark</option></select></label>{platform === "macos" && <div className="permission-setting"><div><span className="field-label">Full Disk Access</span><p className="form-note">One-time setup can improve visibility into protected storage locations. FilePilot cannot confirm the macOS setting itself, and it never grants administrator access.</p><p className="permission-state">{settings.fullDiskAccessSetupComplete ? "Setup acknowledged" : "Not set up yet"}</p></div><button className="button secondary" onClick={() => void openFullDiskAccessSettings()}>Open Full Disk Access</button></div>}<div><span className="field-label">Recent folders</span><p className="form-note">Stored locally so you can return to your workspaces faster. FilePilot keeps the eight most recent paths.</p>{settings.recentPaths.length ? <ul className="recent-list">{settings.recentPaths.map((path) => <li key={path}>{path}</li>)}</ul> : <p className="muted">No recent folders yet.</p>}</div><button className="button primary" onClick={() => api.saveSettings(settings).then(() => setNotice("Settings saved locally")).catch((reason) => setError(String(reason)))}>Save settings</button></section></>; }
...
function storageCategoryLabel(category: StorageItem["category"]) { return ({ "app-caches": "App caches", "app-logs": "App logs", "temporary-files": "Temporary files", "developer-artifacts": "Developer artifacts", "device-backups": "Device backups", "app-support-data": "App support data", "sandboxed-app-data": "Sandboxed app data", "app-settings": "App settings", "mail-data": "Mail data", "messages-data": "Messages data", "cloud-files": "Cloud files", "personal-media": "Personal media", "virtual-memory": "Virtual memory", "system-databases": "System databases", "system-managed": "System managed", "other-system-working-data": "Other system data", "other-data": "Other data" } as Record<StorageItem["category"], string>)[category]; }
